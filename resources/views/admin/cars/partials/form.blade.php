<div class="row">
    <div class="col-xs-12 col-xl-9">
        <ul class="nav nav-tabs" role="tablist">
            ...
            <li class="hidden-xl"><a href="#tabProperties" data-toggle="tab">{{ __('admin-news::news.Settings') }}</a>
            </li>
        </ul>
    </div>
</div>

{!! FormHelper::horizontal(['model' => $car, 'store' => 'admin.cars.store', 'update' => 'admin.cars.update', 'id' => 'carForm']); !!}
...
<div class="row tab-content">
    ...
    <div id="tabBrands" class="col-xs-12 col-xl-9 tab-pane active">

            {!! FormHelper::select('brand_id', 'Merk', $brands, isset($product->brand) ? $product->brand->id : null) !!}

    </div>

    <div id="tabImages" class="col-xs-12 col-xl-9 tab-pane">
        {!! FormHelper::uploader('image', $car->media('image')) !!}
    </div>
    ...
    @if(setting('news.headers'))
        <div id="tabHeaders" class="col-xs-12 col-xl-9 tab-pane">
            {!! FormHelper::headers('headers', $car->headers) !!}
        </div>
    @endif
    ...
    <div id="tabProperties" class="col-xs-12 col-xl-3 tab-pane">
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                ...
                {!! FormHelper::toggle('active', 'Actief', (!$car->exists ? true : null)) !!}
            </div>
        </div>
    </div>
...
<div class="form-footer">
    ...
    <button type="submit" class="btn btn-primary pull-right submit"
            data-type="save_return">{{ __('admin-core::core.Save and back') }}</button>
    <button type="submit" class="btn btn-primary pull-right submit" style="margin-right: 15px;"
            data-type="save">{{ __('admin-core::core.Save') }}</button>
    <a href="{{redirect()->route('admin.cars.index')->getTargetUrl()}}" class="pull-right"
       style="margin: 7px 20px 0 0;">{{ __('admin-core::core.Cancel') }}</a>
</div>
